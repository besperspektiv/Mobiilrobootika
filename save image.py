...
cv.createTrackbar('R1', 'mask', 255, 255, nothing)
cv.createTrackbar('G1', 'mask', 255, 255, nothing)
cv.createTrackbar('B1', 'mask', 255, 255, nothing)

configure_flag = input("Configure robot mask? (Y/N)")
if configure_flag == "Y" or configure_flag == "y":
    configure_flag = True
else:
...
def configure_robot_mask():
    ...
    R1 = cv.getTrackbarPos('R1', 'mask')
    G1 = cv.getTrackbarPos('G1', 'mask')
    B1 = cv.getTrackbarPos('B1', 'mask')

    lower_colour = np.array([R, G, B])  # BRG
    upper_colour = np.array([R1, G1, B1])
...
while True:
    if not configure_flag:
        if not configured:
            ...
            lower_pink = np.array([82, 48, 0])  # BRG
            upper_pink = np.array([167, 121, 67])
        ...
        if result is None or ret == True:
            ...
            to_mask = process_image(image_coppy)
            mask = cv.inRange(to_mask, lower_pink, upper_pink)

            dilation = cv.dilate(mask, kernel, iterations=1)
            opening = cv.morphologyEx(dilation, cv.MORPH_OPEN, kernel)

            # Bitwise-AND mask and original image
            res = cv.bitwise_and(cropped_image, cropped_image, mask=opening)
        ...
        lower_pink, upper_pink = configure_robot_mask()

        if cv.waitKey(1) == ord('c'):
            cv.destroyWindow("mask")
            configured = True
            configure_flag = False
            print(lower_pink, upper_pink)
